...
- `api_surface.json`: 세 FastAPI source의 33개 handler·38개 decorator route, 역할·선택된
...
- `api_effects.json`: 같은 33개 handler의 회원/기업 DB·감사·Redis·agent·gateway·prompt 기록
  효과와 주요 분기를 source review 선언으로 정리하고, 전 handler와 14개 helper의 AST 지문 및
  추천·기업 가입·열람 감사·gateway의 선택된 9개 lexical 순서를 검사한다. CFG dominance,
  실제 cardinality, commit 원자성, downstream 수신 또는 실행 증거가 아니다.
- `api_wire_shapes.json`: 같은 33개 handler의 직접 `return` 표현식, 38개 route의 선언된 성공
