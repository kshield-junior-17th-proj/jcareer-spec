...
const required = [
  ...
  [app.includes('subjectLabel = ""') && app.includes('className="sr-only"> · {subjectLabel}') && candidateRecommendations.includes("data.items.map((item, index)") && candidateRecommendations.includes("조건 일치 결과 ${index + 1}") && candidateRecommendations.split("subjectLabel={subjectLabel}").length === 3 && recruiterRecommendations.includes("data.items.map((item, index)") && recruiterRecommendations.includes("조건 일치 결과 ${index + 1}") && recruiterRecommendations.split("subjectLabel={subjectLabel}").length === 3 && pipelineSurface.includes("{candidate.display_name} · {candidate.email}"), "repeated recommendation and candidate disclosures have unique visible-list names"],
