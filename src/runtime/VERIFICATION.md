...
- 이후 추가된 TRACE·JC-RECEIPT는 독립 임시 SQLite에서 API 계약 6/6, React 정적 계약 11/11을,
  Slack·Notion·SMTP 어댑터는 외부 네트워크를 대역으로 바꾼 계약 18/18을 통과했다. 이는 소스와
  합성 경계 확인이며 Compose 종단간 실행, 실제 credential·외부 전송 또는 AWS 배포 증거가 아니다.
...
- 제외: AWS 배포, 2-AZ 장애조치, RDS 백업·S3 버전, 실제 Bedrock 운영 연동, 실제 지원자 데이터,
  TRACE·외부 업무도구의 실제 계정 연결·전송·운영 활성화
...
| AWS 다이어그램 원본·PNG | 공개 기준 도면 `terraform/asis/JCAREER_ASIS_FLOW.drawio`는 60개 셀·14개 연결이며 같은 이름의 2400×1400 PNG와 함께 검사한다. 77개 셀·23개 연결은 원본 작업 트리의 별도 상세 도면 기록으로, 공개 기준 도면 검사 수에 합치지 않는다. 공개 도면은 6단계 기준 AWS 흐름, 기준 110개와 분리된 default-off MLOps 흐름, 업무망 선언·Slack 외부 SaaS 경계와 기본 비활성 로컬 확장 메모를 보여 준다. Amazon ECR은 공식 AWS 아이콘으로 이미지 저장소 자리와 미게시 상태를 구분한다. MLOps의 추가 여섯 연결은 별도 0/13/14 계획을 설명하며 AWS 실행 증거가 아니다. TRACE·JC-RECEIPT와 Slack·Notion·SMTP 어댑터 메모도 새 AWS 리소스나 실제 외부 전송을 뜻하지 않는다. |
