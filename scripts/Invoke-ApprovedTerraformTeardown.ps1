...
$rootMap = @{
    'serverless-opendart' = @{
        ...
        Scope = 'serverless-opendart-teardown'
        RecoveryScope = 'serverless-opendart-recovery-teardown'
        SourceChecker = 'scripts/check_serverless_opendart_static.py'
    }
    'workplace-images' = @{
        Path = 'terraform/workplace-images'
        Scope = 'workplace-windows-image-teardown'
        RecoveryScope = 'workplace-windows-image-recovery-teardown'
        SourceChecker = 'scripts/check_workplace_images_static.py'
    }
    'workplace-endpoints' = @{
        Path = 'terraform/workplace-endpoints'
        Scope = 'workplace-windows-endpoints-teardown'
        RecoveryScope = 'workplace-windows-endpoints-recovery-teardown'
        SourceChecker = 'scripts/check_workplace_endpoints_static.py'
    }
}
$selected = $rootMap[$Root]
$teardownScope = if ($Recovery) { $selected.RecoveryScope } else { $selected.Scope }
...
$savedPlanFile = Microsoft.PowerShell.Management\Join-Path $workDir 'jcareer-teardown.tfplan'
$planFile = $savedPlanFile
$planJson = Microsoft.PowerShell.Management\Join-Path $workDir 'jcareer-teardown.json'
$receiptFile = Microsoft.PowerShell.Management\Join-Path $workDir 'last-teardown-receipt.json'
$planBindingFile = Microsoft.PowerShell.Management\Join-Path $workDir 'jcareer-teardown.bindings.json'
...
function Copy-ProtectedStableFile {
    ...
    if (-not [IO.File]::Exists($Source)) { throw 'A required teardown input is unavailable.' }
    ...
    if (-not $destinationPath.StartsWith($snapshotPrefix, [StringComparison]::OrdinalIgnoreCase)) {
        throw 'A teardown input snapshot path escaped its protected directory.'
    }
    $sourceStream = $null
    ...
    try {
        ...
        if ([long]$destinationStream.Length -ne $sourceLength) { throw 'A teardown input snapshot length changed.' }
        ...
        if ($sourcePreHash -ne $snapshotHash -or $sourcePreHash -ne $sourcePostHash) {
            throw 'A teardown input changed during protected snapshot capture.'
        }
        ...
        if ([long]$snapshotReadLock.Length -ne $sourceLength) {
            throw 'A teardown input snapshot length changed before read locking.'
        }
        $snapshotLockedHash = Get-StreamSha256 -Stream $snapshotReadLock
        if ($snapshotLockedHash -ne $sourcePreHash) {
            throw 'A teardown input snapshot changed before read locking.'
        }
...
function Move-ProtectedOneShotFile {
    ...
    if (-not [IO.File]::Exists($Source)) { throw 'A required one-shot teardown input is unavailable.' }
    ...
    $snapshotPrefix = $script:protectedSnapshotDirectory.TrimEnd('\', '/') + [IO.Path]::DirectorySeparatorChar
    if (-not $destinationPath.StartsWith($snapshotPrefix, [StringComparison]::OrdinalIgnoreCase)) {
        throw 'A one-shot teardown input path escaped its protected directory.'
    }
    if ([IO.File]::Exists($destinationPath)) { throw 'A one-shot teardown input destination already exists.' }
    $sourceReadLock = $null
    $snapshotReadLock = $null
    try {
        ...
        if ([long]$snapshotReadLock.Length -ne $sourceLength) {
            throw 'A consumed one-shot teardown input length changed during its atomic move.'
        }
        $destinationHash = Get-StreamSha256 -Stream $snapshotReadLock
        if ($destinationHash -ne $sourceHash) {
            throw 'A consumed one-shot teardown input changed during its atomic move.'
        }
        $snapshotReadLock.Position = 0
        $script:protectedSnapshotStreams += $snapshotReadLock
        $script:protectedSnapshotCount += 1
...
function Open-VerifiedPlanJsonReadLock {
    ...
    if ($null -ne $script:planJsonReadLock) { throw 'A teardown plan JSON read lock is already active.' }
    ...
    try {
        ...
        if ([long]$readLock.Length -ne $expectedLength) {
            throw 'The generated teardown plan JSON length changed before checker read locking.'
        }
        $lockedHash = Get-StreamSha256 -Stream $readLock
        if ($lockedHash -ne $expectedHash) {
            throw 'The generated teardown plan JSON changed before checker read locking.'
        }
        $readLock.Position = 0
...
function Remove-ProtectedInputSnapshots {
    ...
    if ($errors.Count -ne 0) { throw 'Teardown input snapshot cleanup was not fully observed.' }
...
function Write-NewProtectedTextUtf8NoBom {
    ...
    if ([IO.File]::Exists($fullPath) -or [IO.Directory]::Exists($fullPath)) {
        throw 'An unresolved Terraform teardown journal path already exists.'
    }
...
function Start-OperationJournal {
    ...
    $script:operationJournalDocument = [ordered]@{
        schema_version = 'jcareer-terraform-teardown-operation-journal-v2'
        scope = $teardownScope
...
function Complete-OperationJournal {
    ...
    $receiptArchiveName = 'jcareer-teardown-receipt-' + $operationId + '.json'
    $journalArchiveName = 'jcareer-teardown-operation-' + $operationId + '.completed.json'
    ...
    if ($receiptArchiveHash -ne $ReceiptSha256) {
        throw 'The archived Terraform teardown receipt hash does not match the finalized receipt.'
    }
    ...
    if ($activeJournalHash -ne $expectedJournalHash -or $journalArchiveHash -ne $expectedJournalHash) {
        throw 'The completed Terraform teardown journal archive does not match the active journal.'
    }
    [IO.File]::Delete($operationJournalFile)
    if ([IO.File]::Exists($operationJournalFile)) { throw 'The completed Terraform teardown journal remains present.' }
    $script:operationJournalStarted = $false
    $script:operationJournalDocument = $null
}
...
function Protect-Diagnostic {
    ...
    $protected = $protected -replace '(?i)\b(bucket|key|dynamodb_table|profile|role_arn|workspace_key_prefix|owner|session(?:_?id)?|user(?:_?name)?)(\s*[=:]\s*)(?:"[^"]*"|''[^'']*''|[^\s,]+)', '$1$2[REDACTED_VALUE]'
...
function Invoke-CheckedOutput {
    ...
    try {
        ...
        $stderr = if ([IO.File]::Exists($stderrPath)) { [IO.File]::ReadAllText($stderrPath, $utf8WithoutBom) } else { '' }
        if ($exitCode -ne 0) {
...
try {
    $script:protectedSnapshotDirectory = Microsoft.PowerShell.Management\Join-Path $workDir ('.jcareer-teardown-operation-' + [Guid]::NewGuid().ToString('N'))
    ...
    $teardownCheckerSource = (Microsoft.PowerShell.Management\Resolve-Path (Microsoft.PowerShell.Management\Join-Path $repoRoot 'scripts/check_terraform_teardown_plan.py')).Path
    ...
    $staticCheckerNames = @(
        'check_serverless_opendart_static.py',
        'check_workplace_endpoints_static.py',
        'check_workplace_images_static.py'
    )
    $staticCheckerSources = @{}
    foreach ($checkerName in $staticCheckerNames) {
        $staticCheckerSources[$checkerName] = (
            Microsoft.PowerShell.Management\Resolve-Path (
                Microsoft.PowerShell.Management\Join-Path $repoRoot ('scripts/' + $checkerName)
            )
        ).Path
    }
    ...
    $staticCheckerSnapshots = @{}
    foreach ($checkerName in $staticCheckerNames) {
        $staticCheckerSnapshots[$checkerName] = Copy-ProtectedStableFile `
            -Source $staticCheckerSources[$checkerName] `
            -DestinationName $checkerName
    }
    $sourceCheckerName = [IO.Path]::GetFileName([string]$selected.SourceChecker)
    if (-not $staticCheckerSnapshots.ContainsKey($sourceCheckerName)) {
        throw 'The selected Terraform source checker is not in the teardown import snapshot set.'
    }
    $sourceChecker = $staticCheckerSnapshots[$sourceCheckerName]
    $teardownChecker = Copy-ProtectedStableFile -Source $teardownCheckerSource -DestinationName 'check_terraform_teardown_plan.py'
    $deploymentChecker = Copy-ProtectedStableFile -Source $deploymentCheckerSource -DestinationName 'check_deployment_approval.py'
    if ($script:protectedSnapshotCount -ne 7) {
        throw 'The common teardown input/checker snapshot set is incomplete.'
    }
    $backendSource = $null
    $backendCheckerSource = $null
    $teardownCheckerSource = $null
    $deploymentCheckerSource = $null
    $staticCheckerSources = $null
    $staticCheckerSnapshots = $null
    ...
    if ($canonicalBackendHash -notmatch '^[0-9a-f]{64}$') { throw 'Terraform logical backend identity could not be derived.' }
    ...
    if ($Apply) {
        if (-not (Microsoft.PowerShell.Management\Test-Path -LiteralPath $savedPlanFile -PathType Leaf)) {
            throw 'No saved teardown plan exists. Run plan-only mode first.'
        }
        if (-not (Microsoft.PowerShell.Management\Test-Path -LiteralPath $planBindingFile -PathType Leaf)) {
            throw 'No saved teardown backend/provider-account binding exists. Run plan-only mode first.'
        }
        $planBinding = Move-ProtectedOneShotFile -Source $planBindingFile -DestinationName 'plan-bindings.json'
        $planFile = Move-ProtectedOneShotFile -Source $savedPlanFile -DestinationName 'approved-teardown.tfplan'
    }
    ...
    if (-not $Apply) {
        ...
        $varFileSource = $null
        if ($script:protectedSnapshotCount -ne 8) {
            throw 'The plan-mode teardown snapshot set is incomplete.'
        }
        ...
        Invoke-CheckedCommand { & $script:pythonExecutable -E -s -S -B $sourceChecker --root $repoRoot } 'Terraform source boundary failed.'
        Invoke-CheckedCommand {
            & $script:terraformExecutable -chdir=$($selected.Path) plan -destroy -refresh=false -input=false -lock-timeout=60s "-var-file=$resolvedVarFile" "-out=$planFile"
        } 'Terraform delete-only saved-plan creation failed.'
        $show = @(
            Invoke-CheckedOutput -Failure 'Terraform teardown plan JSON conversion failed.' -Command {
        ...
        try {
            Invoke-CheckedCommand {
                & $script:pythonExecutable -E -s -S -B $teardownChecker --plan $planJson --scope $teardownScope
            } 'Terraform teardown plan is not allowlisted and delete-only.'
        }
        ...
        if (-not [string]::Equals($providerAccountAfterPlanSha256, $providerAccountSha256, [StringComparison]::Ordinal)) {
            throw 'AWS provider account binding changed while creating the saved teardown plan.'
        }
        ...
        Microsoft.PowerShell.Utility\Write-Output "TEARDOWN_PLAN_ONLY=PASS root=$Root scope=$teardownScope"
        ...
        Microsoft.PowerShell.Utility\Write-Output 'No AWS changes were applied. A human must review this exact delete-only plan.'
        return
    }
    ...
    $approvalSource = $null
    if ($script:protectedSnapshotCount -ne 10) {
        throw 'The apply-mode teardown snapshot set is incomplete.'
    }
    ...
    if ($planBindingKeys.Count -ne 3 -or $missingPlanBindingKeys.Count -ne 0) {
        throw 'Saved teardown plan binding keys differ from the exact schema.'
    }
    if ([string]$planBindingDocument.schema_version -cne 'jcareer-terraform-plan-bindings-v1') {
        throw 'Saved teardown plan binding schema is invalid.'
    }
    $recordedBackendHash = [string]$planBindingDocument.backend_config_sha256
    if ($recordedBackendHash -ne $backendConfigHash) {
        throw 'Backend configuration differs from the one used for the saved teardown plan.'
    }
    ...
    if ($recordedProviderAccountHash -notmatch '^[0-9a-f]{64}$' -or $recordedProviderAccountHash -match '^([0-9a-f])\1{63}$') {
        throw 'Saved teardown provider-account binding is empty, malformed, or placeholder-like.'
    }
    $providerAccountSha256 = Get-ProviderAccountSha256
    if (-not [string]::Equals($providerAccountSha256, $recordedProviderAccountHash, [StringComparison]::Ordinal)) {
        throw 'AWS provider account differs from the one used for the saved teardown plan.'
    }
    Invoke-CheckedCommand {
        & $script:terraformExecutable -chdir=$($selected.Path) init -reconfigure -lockfile=readonly -input=false "-backend-config=$resolvedBackendConfig"
    } 'Terraform remote-backend init failed.'
    ...
    try {
        Invoke-CheckedCommand {
            & $script:pythonExecutable -E -s -S -B $teardownChecker --plan $planJson --scope $teardownScope
        } 'Saved teardown plan is not allowlisted and delete-only.'
        Invoke-CheckedCommand {
            & $script:pythonExecutable -E -s -S -B $deploymentChecker --approval $resolvedApproval --scope $teardownScope --plan $planFile --backend-config-sha256 $backendConfigHash --provider-account-sha256 $providerAccountSha256 --require-approved
        } 'Human teardown approval record validation failed.'
    }
    finally { Close-VerifiedPlanJsonReadLock }
    ...
    if (-not [string]::Equals($providerAccountBeforeApplySha256, $providerAccountSha256, [StringComparison]::Ordinal)) {
        throw 'AWS provider account binding changed immediately before Terraform teardown apply.'
    }
    ...
    } 'Terraform teardown saved-plan apply failed.'
    ...
    if (-not [string]::Equals($providerAccountAfterApplySha256, $providerAccountSha256, [StringComparison]::Ordinal)) {
        throw 'AWS provider account binding changed before teardown completion could be recorded.'
    }

    $receipt = [ordered]@{
        schema_version = 'jcareer-redacted-terraform-teardown-receipt-v1'
        scope = $teardownScope
        ...
        result = $null
        resource_identifiers_included = $false
        post_teardown_inventory_observed = $false
    ...
    $receiptResult = if ($snapshotCleanupFailed) {
        'DELETE_ONLY_PLAN_APPLY_COMPLETED_LOCAL_CLEANUP_UNOBSERVED'
    }
    else { 'DELETE_ONLY_PLAN_APPLIED' }
    ...
    if ($snapshotCleanupFailed) {
        throw 'Terraform teardown completed, but protected local input cleanup was not observed; use the receipt and perform human disposition before retry.'
    }
    Microsoft.PowerShell.Utility\Write-Output "DELETE_ONLY_PLAN_APPLIED root=$Root receipt=.terraform/last-teardown-receipt.json"
    Microsoft.PowerShell.Utility\Write-Output 'A separate redacted post-teardown inventory is still required before claiming zero residual resources.'
}
finally {
    try { Close-VerifiedPlanJsonReadLock }
    catch { Microsoft.PowerShell.Utility\Write-Warning 'The protected teardown plan JSON read lock was not cleanly released.' }
    ...
    if (-not $script:localSnapshotCleanupObserved) {
        ...
        catch { Microsoft.PowerShell.Utility\Write-Warning 'Protected teardown input cleanup was not fully observed.' }
    }
