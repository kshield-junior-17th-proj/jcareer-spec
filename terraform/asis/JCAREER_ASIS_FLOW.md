...
[웹 도면](architecture.html)에서는 전체 인프라의 ①~⑥을 보거나 서비스·보조 경로 5개 중
...
MLOps 경로는 기준 110개와 분리된 서버리스 모델 검증 루트를 세 구간으로 줄여 보여 준다.

### 3.1 MLOps 경로를 번호로 따라가기

1. 별도 랩의 합성 회원·기업 자료를 숫자 특징 5개로 줄이고 S3 입력 파일 3개로 준비한다.
2. 담당자가 확인값을 넣어 일회성 Lambda를 시작하면 파일을 검사하고 후보 모델을 학습한다.
3. S3에 결과 파일 6개, DynamoDB에 상태를 남긴 뒤 사람 검토 대기에서 멈춘다.
...
- 서비스 구현 코드는 검증용 합성 데이터를 사용한다. 이전 통합 runner의 AWS 비접속 소스 검사 92건을 통과했다.
...
- MLOps 전용 경계 시험 12건과 합성 파이프라인 시험 14건을 통과했다. AWS 배포와 모델 품질 평가는 포함하지 않는다.
- Bedrock 실험은 별도 코드다. 현재 관련 리소스는 0개이며 기준 110개에 포함되지 않는다.
