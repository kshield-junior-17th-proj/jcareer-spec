...
- 서비스 구현 코드는 검증용 합성 데이터를 사용한다. 공개 릴리스의 Lab·MLOps·OpenDART 검사 6단계를 통과했다.
...
- Bedrock 직접 합성 호출은 별도 AWS 검증 계정에서 확인했다. Lab 적용은 IAM 권한에서 중단돼 정리했으므로 현재 관련 리소스는 0개이며, 애플리케이션 전체 경로는 확인하지 못했다.
