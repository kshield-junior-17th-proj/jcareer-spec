...
const commonCss = `
  ...
  * { box-sizing: border-box; }
  ...
  body::before { position: fixed; inset: 0; z-index: -1; content: ""; opacity: .36; background-image: radial-gradient(#79736b 0.55px, transparent 0.55px); background-size: 10px 10px; }
  ...
  .masthead { color: #fff; background: var(--ink); border-bottom: 4px solid #e87928; }
  .masthead__inner { width: min(1480px, calc(100% - 48px)); margin: auto; }
  .utility { display: flex; align-items: center; justify-content: space-between; min-height: 48px; border-bottom: 1px solid rgba(255,255,255,.15); font: 700 11px/1 var(--mono); letter-spacing: .075em; text-transform: uppercase; }
  ...
  .kicker { margin: 0 0 12px; color: #f2a469; font: 700 12px/1.3 var(--mono); letter-spacing: .08em; text-transform: uppercase; }
  h1 { max-width: 800px; margin: 0; font-size: clamp(2.5rem, 5vw, 4.7rem); line-height: 1.01; letter-spacing: -.055em; text-wrap: balance; }
  .hero-copy { max-width: 760px; margin: 22px 0 0; color: #d5dde2; font-size: 1.04rem; line-height: 1.7; text-wrap: pretty; }
  .hero-links { display: flex; flex-wrap: wrap; gap: 10px 22px; margin: 20px 0 0; }
  .hero-links a { color: #ffd0ad; font-size: .82rem; font-weight: 800; text-decoration-thickness: 1px; }
  .hero-links a:hover { color: #fff; }
  .doc-control { align-self: end; border-top: 1px solid rgba(255,255,255,.25); }
  .doc-control div { display: grid; grid-template-columns: 110px 1fr; gap: 14px; padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,.15); }
  .doc-control dt { color: #9fb0bd; font: 700 11px/1.5 var(--mono); letter-spacing: .04em; }
...
${commonCss}
    ...
    .plate__frame { margin: 0; padding: 18px; overflow: auto; background: var(--surface); border: 1px solid var(--line); box-shadow: var(--shadow); }
    .diagram-stage { position: relative; width: 100%; min-width: 720px; }
    .diagram-stage > a, .diagram-stage img { display: block; width: 100%; }
    .diagram-stage img { height: auto; border: 1px solid var(--line); }
