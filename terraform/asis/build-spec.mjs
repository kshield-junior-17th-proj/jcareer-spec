...
const architectureFlows = {
  overview: {
    ...
    tone: 'modelled',
    markerTone: 'record',
    summary: '하나의 인프라에서 핵심 추천 2개, 공용 AI 기능 2개, 운영·실험 지원 3개가 어느 구간을 함께 쓰는지 봅니다.',
    detailHref: '../../assets/JCAREER_AI_RUNTIME_ACTUAL.svg',
    detailLabel: '현재 AI 실행면 보기',
    stages: [
      { label: '사용자 요청이 웹 진입 구간을 통과', x: 82, y: 365 },
      { label: '추천 엔진이 자료를 비교하고 점수·순위를 확정', x: 1210, y: 365 },
      { label: 'Bedrock이 확정된 근거를 읽기 쉬운 설명으로 변환', x: 1880, y: 535 },
      { label: '합성 과거 결과는 참고자료로만 조회', x: 1600, y: 715 },
      { label: '합성 MLOps는 별도 경로에서 후보 모델을 비교', x: 550, y: 1125 },
      { label: '상태·오류·지연·버전을 운영 기록으로 남김', x: 1060, y: 905 },
      { label: '모델 변경은 사람 검토 대기에서 멈춤', x: 1660, y: 1125 }
    ],
    visuals: [
      { d: 'M82 430H310H520H730H970H1210H1600', tone: 'core', dur: 5.4 },
      { d: 'M1210 430V600H1130H1380H1880', tone: 'local', dur: 4.2 },
      { d: 'M300 1190H550H990H1210H1430H1660', tone: 'experimental', dur: 5.8 },
      { d: 'M1210 430V910H1060V970', tone: 'record', dur: 3.4 }
    ],
    callout: { x: 1470, y: 1080, w: 380, text: '7개 흐름 · 같은 인프라를 함께 사용' },
  ...
  candidate: {
    ...
    tone: 'local',
    markerTone: 'local',
    summary: '구직자 이력서와 열린 공고 여러 건을 비교해 점수와 순위를 먼저 확정하고, 그 근거를 설명으로 붙여 상위 공고를 보여 줍니다.',
    detailHref: 'index.html#section-31',
    detailLabel: '공고 추천 기능 명세 보기',
    stages: [
      { label: '구직자가 이력서를 입력', x: 82, y: 365 },
      { label: '열린 공고 N개를 조회', x: 1600, y: 365 },
      { label: '추천 엔진이 조건 점수와 순위를 확정', x: 1210, y: 505 },
      { label: '권한 통제를 거쳐 Bedrock이 추천 이유를 생성', x: 1380, y: 535 },
      { label: '점수와 설명을 합쳐 상위 공고를 반환', x: 1880, y: 535 }
    ],
    visuals: [
      { d: 'M82 430H310H520H730H970H1210H1600', tone: 'core', dur: 4.8 },
      { d: 'M1210 430V600H1130H1380H1880', tone: 'local', dur: 3.8 },
      { d: 'M1880 600H1210V430H82', tone: 'reply', dur: 4.5 }
    ],
    callout: { x: 1450, y: 305, w: 500, text: '점수·순위 확정 → 설명 생성 → 상위 공고' },
    boundary: '조건 일치도를 설명하는 서비스이며 합격 가능성을 예측하지 않습니다. LLM은 이미 확정된 점수와 순위를 바꿀 수 없습니다.'
  },
  recruiter: {
    title: '채용담당자 지원자 추천',
    status: '핵심 AI 서비스',
    tone: 'local',
    markerTone: 'local',
    summary: '채용담당자가 자기 회사 공고의 지원자 여러 명을 비교해 점수와 순위를 먼저 확정하고, 추천 이유와 원문 근거를 함께 검토합니다.',
    detailHref: 'index.html#section-31',
    detailLabel: '지원자 추천 기능 명세 보기',
    stages: [
      { label: '채용담당자의 회사·공고 권한을 확인', x: 82, y: 365 },
      { label: '해당 공고의 활성 지원자 N명을 조회', x: 1600, y: 365 },
      { label: '추천 엔진이 조건 점수와 순위를 확정', x: 1210, y: 505 },
      { label: '권한 통제를 거쳐 Bedrock이 추천 이유와 원문 근거를 설명', x: 1380, y: 535 },
      { label: '담당자가 후보를 비교하고 최종 판단', x: 1880, y: 535 }
    ],
    visuals: [
      { d: 'M82 430H310H520H730H970H1210H1600', tone: 'core', dur: 4.8 },
      { d: 'M1600 430H1210V600H1130H1380H1880', tone: 'local', dur: 4.0 },
      { d: 'M1880 600H1210V430H82', tone: 'reply', dur: 4.5 }
    ],
    callout: { x: 1430, y: 305, w: 540, text: '공고 지원자만 비교 · 최종 판단은 채용담당자' },
    boundary: '자기 회사 공고에 지원한 활성 후보자만 대상으로 합니다. 추천은 검토 순서를 돕는 기능이며 채용 결정을 자동화하지 않습니다.'
  },
  explanation: {
    title: 'Bedrock 추천 설명 생성',
    status: '공용 AI 기능',
    tone: 'guarded',
    markerTone: 'local',
    summary: '추천 엔진이 확정한 점수와 근거만 LLM Gateway로 보내고, Broker가 허용한 Bedrock 모델이 한국어 설명을 만든 뒤 원래 결과에 붙입니다.',
    detailHref: 'index.html#section-33',
    detailLabel: 'AI 점수·설명 규칙 보기',
    stages: [
      { label: '추천 엔진이 확정 점수·근거를 전달', x: 1210, y: 505 },
      { label: 'LLM Gateway가 설명 전용 요청을 구성', x: 1130, y: 535 },
      { label: 'Capability Broker가 허용 모델만 호출', x: 1380, y: 535 },
      { label: 'Bedrock 설명을 원래 점수 결과에 결합', x: 1880, y: 535 }
    ],
    visuals: [
      { d: 'M1210 430V600H1130H1380H1880', tone: 'local', dur: 3.6 },
      { d: 'M1880 600H1210V430', tone: 'reply', dur: 3.3 }
    ],
    callout: { x: 1160, y: 660, w: 760, text: 'Capability Broker는 서비스가 아니라 Bedrock 권한 통제 구성요소' },
  ...
  values: {
    title: '기업 방향·가치 일치 근거',
    status: '공용 AI 기능',
    tone: 'guarded',
    markerTone: 'local',
    summary: '자소서·프로젝트와 기업이 공개한 방향·가치에서 실제로 같은 문구가 나온 위치를 찾고, Bedrock이 사람이 읽기 쉬운 설명으로 바꿉니다.',
    detailHref: 'index.html#section-31',
    detailLabel: '기업 방향 대조 규칙 보기',
    stages: [
      { label: '자소서·프로젝트와 기업 선언 자료를 조회', x: 1600, y: 365 },
      { label: '같은 문구가 있는 위치를 직접 추출', x: 1210, y: 505 },
      { label: 'Gateway가 추출된 근거만 설명 요청으로 구성', x: 1130, y: 535 },
      { label: 'Bedrock이 사람이 읽을 설명으로 변환', x: 1880, y: 535 },
      { label: '설명을 참고 근거로 표시하고 점수에는 미반영', x: 82, y: 365 }
    ],
    visuals: [
      { d: 'M1600 430H1210V600H1130H1380H1880', tone: 'local', dur: 4.1 },
      { d: 'M1880 600H1210V430H82', tone: 'reply', dur: 4.3 }
    ],
    callout: { x: 1390, y: 305, w: 590, text: '직접 일치 문구만 설명 · 의미 기반 적합도 판정 아님' },
    boundary: '이 기능은 의미 기반 기업 적합도 판정이 아닙니다. 같은 문구와 그 위치를 설명할 뿐이며 추천 점수·순위에 영향을 주지 않습니다.'
  },
  history: {
    title: '합성 과거 결과 관찰',
    status: '운영·실험 지원',
    tone: 'guarded',
    markerTone: 'record',
    summary: '합성 지원 결과에서 비슷한 특징과 과거 관찰을 조회해 추천 화면의 참고자료로만 보여 주는 흐름입니다.',
    detailHref: 'index.html#section-31',
    detailLabel: '합성 결과 관찰 범위 보기',
    stages: [
      { label: '합성 지원 결과 자료를 조회', x: 1600, y: 365 },
      { label: '현재 요청과 비슷한 숫자 특징을 비교', x: 1600, y: 715 },
      { label: '과거 관찰을 참고자료로 구성', x: 1210, y: 505 },
      { label: '추천 화면에 참고 표시로만 반환', x: 82, y: 365 }
    ],
    visuals: [
      { d: 'M82 430H310H520H730H970H1210H1600V770', tone: 'record', dur: 5.0 },
      { d: 'M1600 770V430H1210H82', tone: 'reply', dur: 4.6 }
    ],
    callout: { x: 1370, y: 825, w: 470, text: '참고자료 전용 · 운영 점수·합격 확률 미사용' },
    boundary: '운영 점수와 합격 확률에는 사용하지 않습니다. 합성 결과 관찰을 실제 채용 성과나 통계적 예측으로 해석하면 안 됩니다.'
  },
  mlops: {
    title: '합성 MLOps Challenger',
    status: '운영·실험 지원',
    tone: 'guarded',
    markerTone: 'experimental',
    summary: '합성 회원·기업 자료에서 개인정보를 뺀 숫자 특징 5개를 만들고, 별도 Lambda가 후보 모델을 학습·비교한 뒤 사람 검토 대기에서 멈춥니다.',
    detailHref: '../../mlops/',
    detailLabel: 'MLOps 7단계 상세 보기',
    stages: [
      { label: '합성 회원·기업 자료를 별도 입력으로 준비', x: 300, y: 1125 },
      { label: '개인정보를 제외한 숫자 특징 5개 생성', x: 550, y: 1125 },
      { label: 'S3에 실행별 Snapshot을 보관', x: 780, y: 1125 },
      { label: 'MLOps Lambda가 후보 모델을 학습·비교', x: 990, y: 1125 },
      { label: '결과와 실행 상태를 S3·DynamoDB에 기록', x: 1320, y: 1125 },
      { label: 'TRAINED_PENDING_HUMAN_REVIEW에서 정지', x: 1660, y: 1125 }
    ],
    visuals: [
      { d: 'M300 1190H550H990H1210H1430H1660', tone: 'experimental', dur: 5.4 },
      { d: 'M780 1330V1280H990V1190', tone: 'experimental', dur: 2.8 },
      { d: 'M1200 1330V1280H990V1190', tone: 'record', dur: 3.1 }
    ],
    callout: { x: 1150, y: 1030, w: 650, text: '후보 모델은 운영 매처에 자동 적용되지 않음' },
  ...
  audit: {
    ...
    tone: 'modelled',
    markerTone: 'record',
    summary: '추천 처리기·LLM 게이트웨이 상태, 모델·산식 버전, 모델 제공자 설정 지문, 오류·지연·프롬프트 필드를 묶어 관리자 화면과 검토 증적으로 전달합니다.',
    detailHref: 'index.html#section-52',
    detailLabel: 'AI 운영·감사 명세 보기',
    stages: [
      { label: '추천 처리기·LLM 게이트웨이 상태를 수집', x: 1210, y: 505 },
      { label: '모델·산식 버전과 모델 제공자 설정 지문을 결합', x: 1130, y: 535 },
      { label: '오류·지연·허용된 프롬프트 필드를 기록', x: 1060, y: 905 },
      { label: 'CloudWatch·S3에 운영 증적을 보관', x: 1300, y: 905 },
      { label: '관리자 AI 운영 화면에서 사람이 검토', x: 1540, y: 905 }
    ],
    visuals: [
      { d: 'M1210 430V910H1060V970', tone: 'record', dur: 3.7 },
      { d: 'M1130 600V850H1060V970H1300', tone: 'local', dur: 3.6 },
      { d: 'M1540 970H1300', tone: 'record', dur: 2.6 }
    ],
    callout: { x: 1120, y: 1035, w: 700, text: '관찰 기록은 자동 적합성 판정이나 인증 결론이 아님' },
    boundary: '로그에는 원문 개인정보나 비밀값을 남기지 않아야 합니다. 기록 존재만으로 통제 효과·적합성·인증 가능성을 단정하지 않고 사람이 증적 범위를 확인합니다.'
  }
};

const svgStepMarkers = (flowKey) => {
  const flow = architectureFlows[flowKey];
  if (!flow) return '';
  const tone = flow.markerTone || '';
  return flow.stages.filter(({ x, y }) => Number.isFinite(x) && Number.isFinite(y)).map(({ x, y }, index) => `
            <g class="flow-step-marker ${tone}" data-flow-step="${index + 1}" transform="translate(${x} ${y})">
              <circle class="flow-marker ${tone}" r="18" />
              <text class="flow-marker-text" y="1">${index + 1}</text>
            </g>`).join('');
};

const svgFlowLayer = (flowKey, active = false) => {
  const flow = architectureFlows[flowKey];
  const paths = (flow.visuals || []).map(({ d, tone = 'core', dur = 4 }) => `
              <path class="flow-line ${tone}" d="${escapeHtml(d)}" />
              <circle class="flow-packet ${tone}" r="9">
                <animateMotion dur="${dur}s" repeatCount="indefinite" path="${escapeHtml(d)}" />
              </circle>`).join('');
  const callout = flow.callout ? `
              <g class="flow-callout-group">
                <rect class="flow-callout ${flow.markerTone || ''}" x="${flow.callout.x}" y="${flow.callout.y}" width="${flow.callout.w}" height="48" rx="4" />
                <text class="flow-callout-text" x="${flow.callout.x + flow.callout.w / 2}" y="${flow.callout.y + 31}">${escapeHtml(flow.callout.text)}</text>
              </g>` : '';
  return `<g class="flow-layer${active ? ' is-active' : ''}" data-flow-layer="${flowKey}">${paths}${callout}${svgStepMarkers(flowKey)}
            </g>`;
};
...
const commonCss = `
  ...
  * { box-sizing: border-box; }
  ...
  .document { min-width: 0; background: var(--surface); border: 1px solid var(--line); box-shadow: 0 16px 42px rgba(45,49,47,.08); }
  .executive { display: grid; grid-template-columns: minmax(0, 1.35fr) minmax(270px, .65fr); gap: 28px; padding: clamp(28px, 5vw, 60px); background: #fffdf8; border-bottom: 1px solid var(--line); }
...
${commonCss}
    ...
    .axis-rail { margin: 0 0 18px; display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); background: rgba(255,255,255,.46); border: 1px solid var(--line); box-shadow: none; }
    ...
    .architecture-workspace { display: grid; grid-template-columns: minmax(330px, .52fr) minmax(0, 1.48fr); gap: 18px; align-items: start; }
    .flow-explorer { margin: 0; padding: 25px; color: var(--ink); background: #f4f0e7; border: 1px solid var(--line); border-top: 2px solid var(--ink); box-shadow: none; }
    .flow-explorer__head { display: grid; grid-template-columns: 1fr; gap: 12px; align-items: end; }
    .flow-explorer__head h2 { max-width: 760px; margin: 0; font-size: clamp(1.55rem, 2.7vw, 2.2rem); line-height: 1.12; letter-spacing: -.04em; text-wrap: balance; }
    .flow-explorer__head > p { margin: 0; color: #59666e; font-size: .82rem; line-height: 1.65; }
    .flow-selector { margin-top: 20px; display: grid; grid-template-columns: 1fr; gap: 0; border-top: 1px solid var(--line); }
    .flow-group-label { margin: 15px 0 0; padding: 0 12px 7px; color: #6c777d; border-bottom: 1px solid var(--line); font: 750 .62rem/1.25 var(--mono); letter-spacing: .08em; }
    .flow-button { min-height: 58px; padding: 9px 10px; display: grid; grid-template-columns: 28px minmax(0, 1fr); justify-content: stretch; align-items: center; gap: 10px; border: 0; border-bottom: 1px solid var(--line); color: var(--ink); background: transparent; font-family: inherit; text-align: left; cursor: pointer; transition: color .2s, background .2s, box-shadow .2s, transform .2s; }
    .flow-button:hover { color: var(--accent); background: rgba(255,255,255,.46); transform: none; }
    .flow-button:active { transform: translateY(1px); }
    .flow-button:focus-visible { outline: 3px solid rgba(197,83,22,.32); outline-offset: -3px; }
    .flow-button[aria-pressed="true"] { color: var(--ink); background: #fffdf8; box-shadow: inset 3px 0 0 var(--accent); }
    .flow-button__index { display: grid; width: 25px; height: 25px; place-items: center; color: #68747b; border: 1px solid #bfc4c1; font: 800 .63rem/1 var(--mono); font-variant-numeric: tabular-nums; }
    .flow-button[aria-pressed="true"] .flow-button__index { color: #fff; background: var(--accent); border-color: var(--accent); }
    .flow-button__copy { min-width: 0; }
    .flow-button strong { display: block; font-size: .86rem; line-height: 1.3; }
    .flow-button small { display: block; margin-top: 3px; color: #748087; font: 700 .61rem/1.25 var(--mono); }
    .flow-button[aria-pressed="true"] small { color: #65717a; }
    .flow-detail { margin-top: 18px; padding: 19px 0 0; display: grid; grid-template-columns: 1fr; gap: 18px; color: var(--ink); background: transparent; border-top: 2px solid var(--ink); view-transition-name: flow-detail; }
    ...
    .flow-steps { margin: 0; padding: 0; display: grid; gap: 0; list-style: none; border-top: 1px solid var(--line); }
    ...
    .flow-explorer__exclusion { margin: 17px 0 0; padding-top: 13px; color: #68747b; border-top: 1px solid var(--line); font-size: .7rem; }
    .plate__frame { position: relative; margin: 0; padding: 12px; overflow: auto; background: #ece8df; border: 1px solid var(--line); box-shadow: 0 18px 46px rgba(45,49,47,.1); }
    ...
    .diagram-stage img { height: auto; border: 1px solid var(--line); filter: saturate(.88) contrast(1.02); opacity: 1; transition: filter .3s ease, opacity .3s ease; }
    .diagram-stage--full img { filter: saturate(.94) contrast(1.02); box-shadow: none; }
    .plate__frame:hover .diagram-stage img { filter: saturate(.94) contrast(1.03); }
    .plate__frame[data-active-flow]:not([data-active-flow="overview"]) .diagram-stage img { filter: grayscale(.78) saturate(.42) contrast(.92); opacity: .42; }
    .flow-overlay { position: absolute; inset: 0; z-index: 2; width: 100%; height: 100%; pointer-events: none; }
    .flow-layer { opacity: 0; transition: opacity .25s ease; }
    .flow-layer.is-active { opacity: 1; }
    .flow-line { fill: none; stroke: #c9541f; stroke-width: 10; stroke-linecap: round; stroke-linejoin: round; opacity: .92; stroke-dasharray: 28 22; animation: flowMarch 2.4s linear infinite; filter: drop-shadow(0 2px 3px rgba(58,54,44,.2)); }
    .flow-line.core { stroke: #c9541f; }
    .flow-line.local { stroke: #0d748c; stroke-dasharray: 16 18; }
    .flow-line.reply { stroke: #087f72; stroke-dasharray: 12 16; }
    .flow-line.experimental { stroke: #7b4c91; stroke-dasharray: 9 15; }
    .flow-line.missing { stroke: #9b4b8f; stroke-dasharray: 6 20; }
    .flow-line.record { stroke: #7a5b1e; stroke-dasharray: 28 22; }
    .flow-node { fill: rgba(255,255,255,.08); stroke: #c9541f; stroke-width: 5; }
    .flow-node.local { stroke: #0d7f9b; }
    .flow-node.missing { stroke: #9b4b8f; stroke-dasharray: 9 10; }
    .flow-node.record { stroke: #7a5b1e; stroke-dasharray: 9 10; }
    .flow-marker { fill: var(--ink); stroke: #fff; stroke-width: 2; }
    .flow-marker.local { fill: #0d7f9b; }
    .flow-marker.missing { fill: #8c2e8f; }
    .flow-marker.record { fill: #765313; }
    .flow-marker.experimental { fill: #6f4788; }
    .flow-marker.unknown { fill: #5a6c86; }
    .flow-marker-text { fill: #fff; font: 800 16px/1 var(--sans); text-anchor: middle; dominant-baseline: central; }
    .flow-step-marker { transform-box: fill-box; transform-origin: center; }
    .flow-packet { fill: #c9541f; stroke: #fff; stroke-width: 3; filter: drop-shadow(0 3px 5px rgba(32,43,53,.38)); }
    .flow-packet.local { fill: #0d748c; }
    .flow-packet.reply { fill: #087f72; }
    .flow-packet.experimental { fill: #7b4c91; }
    .flow-packet.record { fill: #765313; }
    .flow-callout { fill: rgba(255,255,255,.9); stroke: #0d748c; stroke-width: 2; }
    .flow-callout.missing { stroke: #9b4b8f; stroke-dasharray: 9 10; }
    .flow-callout.unknown { fill: none; stroke: #5a6c86; stroke-width: 2; stroke-dasharray: 9 10; }
    .flow-callout-text { fill: var(--ink); font: 750 18px/1.2 var(--sans); text-anchor: middle; }
    ...
    .legend-line { width: 32px; border-top: 2px solid #c9541f; }
    ...
    html[data-motion="reduced"] .flow-packet, html.document-hidden .flow-packet { display: none; }
    ...
    @media (max-width: 1200px) { .architecture-workspace { grid-template-columns: 1fr; } .flow-explorer__head { grid-template-columns: minmax(0, 1fr) minmax(260px, .46fr); gap: 36px; } .flow-selector { grid-template-columns: repeat(4, minmax(0, 1fr)); border-left: 1px solid var(--line); } .flow-group-label { grid-column: 1 / -1; } .flow-button { border-right: 1px solid var(--line); } .flow-detail { grid-template-columns: minmax(250px, .65fr) minmax(0, 1.35fr); gap: 30px; } .flow-detail__boundary { grid-column: 1 / -1; } }
    ...
    @media (max-width: 640px) { .plate { width: calc(100% - 24px); margin-top: 20px; } .plate__frame { padding: 6px; } .flow-explorer { padding: 22px 16px; } .flow-selector { grid-template-columns: 1fr 1fr; } .flow-button { min-height: 60px; } .flow-detail { padding-top: 18px; gap: 20px; } .flow-detail__boundary { grid-column: auto; } .diagram-stage { min-width: 900px; } .diagram-stage--full { min-width: 980px; } }
...
<body>
  ...
  <main class="plate" id="diagram">
    ...
    <section class="flow-explorer" id="ai-service-flows" aria-labelledby="flow-explorer-title">
      ...
      <div class="flow-selector" role="group" aria-label="표시할 AI 서비스와 기능">
        <button class="flow-button" type="button" data-flow-button="overview" aria-pressed="true" aria-controls="flow-detail"><span class="flow-button__index">ALL</span><span class="flow-button__copy"><strong>AI 서비스 전체 흐름</strong><small>7개 경로 한눈에 보기</small></span></button>
        <p class="flow-group-label">사용자 대상 핵심 AI 서비스</p>
        <button class="flow-button" type="button" data-flow-button="candidate" aria-pressed="false" aria-controls="flow-detail"><span class="flow-button__index">01</span><span class="flow-button__copy"><strong>구직자 공고 추천</strong><small>이력서 → 상위 공고</small></span></button>
        <button class="flow-button" type="button" data-flow-button="recruiter" aria-pressed="false" aria-controls="flow-detail"><span class="flow-button__index">02</span><span class="flow-button__copy"><strong>채용담당자 지원자 추천</strong><small>공고 → 담당자 검토</small></span></button>
        <p class="flow-group-label">두 서비스가 함께 쓰는 AI 기능</p>
        <button class="flow-button" type="button" data-flow-button="explanation" aria-pressed="false" aria-controls="flow-detail"><span class="flow-button__index">03</span><span class="flow-button__copy"><strong>Bedrock 추천 설명 생성</strong><small>확정 근거 → 한국어 설명</small></span></button>
        <button class="flow-button" type="button" data-flow-button="values" aria-pressed="false" aria-controls="flow-detail"><span class="flow-button__index">04</span><span class="flow-button__copy"><strong>기업 방향·가치 일치 근거</strong><small>같은 문구의 위치만 설명</small></span></button>
        <p class="flow-group-label">AI 운영·실험 지원</p>
        <button class="flow-button" type="button" data-flow-button="history" aria-pressed="false" aria-controls="flow-detail"><span class="flow-button__index">05</span><span class="flow-button__copy"><strong>합성 과거 결과 관찰</strong><small>참고자료 전용</small></span></button>
        <button class="flow-button" type="button" data-flow-button="mlops" aria-pressed="false" aria-controls="flow-detail"><span class="flow-button__index">06</span><span class="flow-button__copy"><strong>합성 MLOps Challenger</strong><small>학습·비교 → 사람 검토 대기</small></span></button>
        <button class="flow-button" type="button" data-flow-button="audit" aria-pressed="false" aria-controls="flow-detail"><span class="flow-button__index">07</span><span class="flow-button__copy"><strong>AI 운영 관찰·감사</strong><small>상태·버전·오류·지연 기록</small></span></button>
      </div>
      ...
      <p class="flow-explorer__exclusion">OpenDART는 AI 서비스가 아니라 기업 공개정보 보강 경로입니다. Capability Broker도 서비스가 아니라 Bedrock 호출 권한을 제한하는 구성요소입니다. 둘 다 관련 흐름 안에서만 표시합니다.</p>
    </section>
    <figure class="plate__frame" id="diagram-frame" aria-describedby="flow-boundary diagram-caption" data-active-media="overview">
      <div class="diagram-media" data-flow-media="overview" aria-hidden="false">
        <div class="diagram-stage diagram-stage--full" data-full-map>
          ...
          <svg class="flow-overlay" viewBox="0 0 2320 1500" preserveAspectRatio="xMidYMid meet" aria-hidden="true" focusable="false">
            ${svgFlowLayer('overview', true)}
            ${svgFlowLayer('candidate')}
            ${svgFlowLayer('recruiter')}
            ${svgFlowLayer('explanation')}
            ${svgFlowLayer('values')}
            ${svgFlowLayer('history')}
            ${svgFlowLayer('mlops')}
            ${svgFlowLayer('audit')}
          </svg>
        </div>
      ...
      <div class="diagram-media" data-flow-media="asis" aria-hidden="true" hidden>
        <div class="diagram-stage">
          ...
          <svg class="flow-overlay" viewBox="0 0 2400 1400" preserveAspectRatio="xMidYMid meet" aria-hidden="true" focusable="false">
            <g class="flow-layer is-active" data-legacy-flow-layer="overview">
              ...
              <circle class="flow-node" cx="435" cy="435" r="44" /><circle class="flow-node" cx="680" cy="435" r="44" /><circle class="flow-node" cx="925" cy="455" r="46" /><circle class="flow-node" cx="1280" cy="510" r="44" /><circle class="flow-node" cx="1600" cy="510" r="46" />
            </g>
            <g class="flow-layer" data-legacy-flow-layer="candidate">
              <path class="flow-line" d="M165 455H382 M490 455H628 M734 455H870 M980 455H1120V510H1216 M1348 510H1535" />
              <circle class="flow-node local" cx="1600" cy="510" r="50" />
              <rect class="flow-callout" x="1370" y="337" width="460" height="48" rx="2" /><text class="flow-callout-text" x="1600" y="368">서비스 구현 범위 · 데이터 연계 검토</text>
${svgStepMarkers('candidate', 'local')}
            </g>
            <g class="flow-layer" data-legacy-flow-layer="recruiter">
              <path class="flow-line" d="M165 455H382 M490 455H628 M734 455H870 M980 455H1120V510H1216 M1348 510H1535" />
              <circle class="flow-node local" cx="1600" cy="510" r="50" />
              <rect class="flow-callout" x="1370" y="337" width="460" height="48" rx="2" /><text class="flow-callout-text" x="1600" y="368">서비스 구현 범위 · 데이터 연계 검토</text>
${svgStepMarkers('recruiter', 'local')}
            </g>
            <g class="flow-layer" data-legacy-flow-layer="explanation">
              <path class="flow-line" d="M165 455H382 M490 455H628 M734 455H870 M980 455H1120V510H1216 M1348 510H1535" />
              <circle class="flow-node local" cx="1600" cy="510" r="50" />
              <rect class="flow-callout" x="1405" y="337" width="390" height="48" rx="2" /><text class="flow-callout-text" x="1600" y="368">점수 고정 → 설명만 생성</text>
${svgStepMarkers('explanation', 'local')}
            </g>
            <g class="flow-layer" data-legacy-flow-layer="mlops"></g>
            <g class="flow-layer" data-legacy-flow-layer="workplace">
              <rect class="flow-callout unknown" x="25" y="640" width="290" height="710" rx="18" />
${svgStepMarkers('workplace', 'unknown')}
            </g>
            <g class="flow-layer" data-legacy-flow-layer="trace">
              <path class="flow-line local" d="M1500 565H1870" />
              <circle class="flow-node local" cx="1600" cy="510" r="52" />
              <rect class="flow-callout" x="1360" y="657" width="520" height="48" rx="2" /><text class="flow-callout-text" x="1620" y="688">기본 비활성 · receipt와 사람 검토 소스</text>
${svgStepMarkers('trace', 'local')}
            </g>
            <g class="flow-layer" data-legacy-flow-layer="integrations">
              <path class="flow-line local" d="M1450 610H1850" />
              <circle class="flow-node local" cx="1600" cy="510" r="52" />
              <rect class="flow-callout" x="1360" y="657" width="520" height="48" rx="2" /><text class="flow-callout-text" x="1620" y="688">opt-in 소스 · 실제 외부 전송 미확인</text>
${svgStepMarkers('integrations', 'local')}
            </g>
            <g class="flow-layer" data-legacy-flow-layer="operations">
              <path class="flow-line record" d="M1290 505V742 M1630 505V742" />
              <circle class="flow-node record" cx="572" cy="775" r="38" /><circle class="flow-node record" cx="1292" cy="775" r="38" /><circle class="flow-node record" cx="1632" cy="775" r="38" /><circle class="flow-node record" cx="1992" cy="775" r="38" />
  ...
  <script>
    ...
    const showFlow = (requestedKey, updateAddress = true) => {
      ...
      const updateFlow = () => {
        ...
        diagramFrame.dataset.activeFlow = key;
